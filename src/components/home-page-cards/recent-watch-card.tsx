...
const RecentWatchCard: React.FC<RecentWatchCardProps> = ({ recentWatch }) => {
  return (<HomePageCard className='items-center content-center justify-items-center justify-content-center'>
    ...
    <div style={{
      ...
      <ul className={cn(styles["gallery"])}>
        {recentWatch?.map((movie, index) => (
          <li className={cn('cursor-pointer', styles['gallery-item'], styles['pic-' + index])}
          ...
          >
            <Image src={movie?.film?.image?.medium || movie?.film?.image?.large || movie?.film?.image?.small || ""} alt={`Dominic recently watched movie - ${movie.title}`} width={120} height={160} />
          </li>
        ))}
      </ul>
    </div>
